...
- 当前是 VIN 步骤
- 当前正处于确认弹窗
- 当前正展示 45 度现场照片或车损拍摄指引弹窗
- 页面离开中
- 相机还未完成初始化

恢复时机：

- 拍照页读取缓存并完成 `setData` 后，会按当前步骤重新尝试恢复检测。
- 相机 `initdone` 回调设置 `cameraInitialized = true` 后，会再次调用检测恢复逻辑。
- VIN 照片确认后切换到车损步骤时，会在步骤数据落定后重新启动车损检测。
- 拍摄指引弹窗关闭后，会恢复当前步骤检测；该弹窗不卸载 `camera` 组件，不走相机重挂载链路。
- 启动前会停止已有检测循环，并通过 `aiDetectionRunId` 阻止旧异步循环继续调度，避免重复检测。
...
- 宽于 4:3 的实时帧按相机预览的 aspect-fill 裁剪逻辑换算，避免宽屏帧检测坐标与视觉取景框错位
- 正方形或窄于 4:3 的实时帧按高度贴合到虚拟相机高度，再左右补边；nova13 日志中的 `480 x 480` 帧会走该分支，避免检测框中心被错误映射到取景框下方
...
- 本版本没有修改车牌或车损 AI 检测、锁定、冷却、自动拍照策略。
- 确认照片后的相册保存是确认态附加动作，不参与 AI 检测循环调度。
- 保存失败不会影响 `resetAIState()`、`resumeAIDetectionAfterStepReady()` 或车损步骤继续检测。
- 重拍仍只恢复当前拍摄步骤和 AI 检测，不触发相册保存。

---

## v1.3.5 与 AI 拍照流程关系

- 本版本不调整车牌、车损 AI 检测策略，也不修改 VIN、车损拍摄状态机。
- 拍照页仅统一背景视觉，AI 覆盖层、检测循环、确认/重拍事件保持原逻辑。
- 驾驶证和行驶证上传在证件信息页完成，使用车辆级证件缓存，不进入 AI 自动拍照流程。
