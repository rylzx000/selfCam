...
小程序应按初始化返回的 `maxCount` 渲染槽位：案件级现场照片 1 个槽位、现场补充照片最多 2 个槽位；车辆级车牌和 VIN 各 1 个槽位，车损按 `maxCount=10` 控制最多 10 张，驾驶证和行驶证按正页、副页或电子证件分别渲染。车辆列表只读展示，三者车不得由小程序新增或删除。
...
| `SCENE_SUPPLEMENT` | 后端按案件级现场补充照片配置映射 | 后端按案件级现场补充照片配置映射 | 2 | 案件级现场补充照片。 |
...
| `sortNo` | Number | 否 | 同一上传项下的排序。车损照片使用 `1..10`，现场补充照片使用 `1..2`；现场主照片、车牌、VIN 和各类证件照片固定为 `1`。 |
...
5. 后端必须校验上传数量不能超过初始化返回的 `maxCount`，例如案件级现场照片 1 张、现场补充照片最多 2 张、同一车辆车损最多 10 张、车牌/VIN/驾驶证/行驶证正副页各 1 张。
