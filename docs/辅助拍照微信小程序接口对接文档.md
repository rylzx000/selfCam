...
| Header | 必填 | 示例 | 说明 |
| --- | --- | --- | --- |
| `Content-Type` | 是 | `application/json;charset=UTF-8` | JSON 请求；图片上传接口使用 `multipart/form-data`。 |
...
sequenceDiagram
    ...
    Mini->>OC: init(ticket)
    OC-->>Mini: 返回事故、车辆、拍照项
    Mini->>OC: uploadPhoto(ticket, file, metadata)
...
图片上传方式已确认：小程序不直传对象存储，也不需要获取对象存储上传凭证。小程序将图片文件上传到 onlineclaim，由后端服务统一处理对象存储、图片模块落库和快赔图片列表同步。
...
| 3 | `POST /onlineclaim/AuxPhotoService/uploadPhoto` | 小程序 | 将图片文件和元数据上传到 onlineclaim，由后端处理图片。 |
...
小程序拍照后，直接将图片文件和图片元数据上传到 onlineclaim。后端服务负责：
...
```http
POST /onlineclaim/AuxPhotoService/uploadPhoto
```
...
一期上传接口按“单张图片一个请求”设计。小程序可以一次选择或拍摄多张图片，但需要在前端按队列逐张调用 `uploadPhoto`；全部上传完成后再调用 `complete`。
...
| --- | --- | --- | --- |
| `file` | File | 是 | 图片文件。 |
| `metadata` | String(JSON) | 是 | 图片元数据，JSON 字符串。 |
...
```javascript
wx.uploadFile({
  url: `${baseUrl}/onlineclaim/AuxPhotoService/uploadPhoto`,
...
| `fileType` | String | 否 | 文件类型，如 `jpg`、`png`。 |
| `fileSize` | Number | 否 | 文件大小，单位 byte。 |
| `fileHash` | String | 强烈建议 | 文件哈希，用于重复上传判断。 |
| `shootTime` | String | 否 | 拍摄时间。 |
...
小程序一期主流程不依赖独立状态查询接口。小程序重新打开页面、上传失败重试、完成后展示结果时，优先重新调用 `init`，由 `init` 返回 `ticketStatus` 和各拍照项 `uploadedCount`。
...
| `AUX_TICKET_COMPLETED` | 辅助拍照已完成 | 进入完成页或只读查看页。 |
...
6. 链接过期、作废、案件关闭时，小程序应阻止继续拍照和上传。
7. 如果客户中途退出后再次打开链接，小程序应重新调用 `init` 恢复已上传状态。
...
11. 行驶证正页和副页是两个独立 `uploadItemId`，不要用一个行驶证上传项承载两张图。
