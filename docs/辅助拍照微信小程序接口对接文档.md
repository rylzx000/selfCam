...
| 槽位 | 数量限制 | 说明 |
| --- | --- | --- |
| 现场照片 | 1 张 | 案件级整车 45 度现场照片，用于记录事故现场和车辆整体状态。 |
| 现场补充照片 | 最多 3 张 | 案件级补充环境照片，可选补拍。 |
| 车牌 | 1 张 | 用于车牌识别或车辆信息核验。 |
| VIN | 1 张 | 用于 VIN 识别或二期车辆定型信息补充。 |
| 车损 | 最多 10 张 | 用于车辆损失查看，并作为二期智能定损图片来源。 |
| 驾驶证 | 实物 2 张或电子 1 张 | 实物驾驶证分为正页和副页；电子驾驶证使用独立上传类型。 |
...
```json
{
  ...
  "data": {
    ...
    "caseUploadItems": [
      {
        "uploadItemId": "CASE_100001_SCENE_45",
        "photoType": "SCENE_45",
        "photoName": "现场照片",
        "required": false,
        "maxCount": 1,
      ...
      {
        "uploadItemId": "CASE_100001_SCENE_SUPPLEMENT",
        "photoType": "SCENE_SUPPLEMENT",
        "photoName": "现场补充照片",
        "required": false,
        "maxCount": 3,
        "uploadedCount": 0
...
初始化响应分为顶层 `caseUploadItems[]` 和 `vehicles[] -> uploadItems[]`：案件级数组只下发现场照片，车辆级数组下发车牌、VIN、车损和车辆证件。小程序不需要感知影像目录编码、文件夹编码或对象存储路径。

本轮最小化接口改造要求后端必须在 `caseUploadItems[]` 下发 `SCENE_45` 和 `SCENE_SUPPLEMENT` 的有效 `uploadItemId`。如果现场照片存在但案件级拍照项未下发，前端按后端配置错误处理并提示“案件级拍照项未下发，请联系后台配置。”，不会静默跳过现场照片上传。

小程序应按初始化返回的 `maxCount` 渲染槽位：案件级现场照片 1 个槽位、现场补充照片最多 3 个槽位；车辆级车牌和 VIN 各 1 个槽位，车损按 `maxCount=10` 控制最多 10 张，驾驶证和行驶证按正页、副页或电子证件分别渲染。车辆列表只读展示，三者车不得由小程序新增或删除。
...
| `caseUploadItems` | Array | 案件级拍照项，仅包含 `SCENE_45` 和 `SCENE_SUPPLEMENT`。 |
| `caseUploadItems[].uploadItemId` | String | 案件级上传项唯一标识，小程序上传时原样回传。 |
| `caseUploadItems[].photoType` | String | 案件级照片类型。 |
| `caseUploadItems[].photoName` | String | 案件级拍照项展示名称。 |
| `caseUploadItems[].required` | Boolean | 是否必拍。当前前端不硬拦截，但会展示缺项提示。 |
| `caseUploadItems[].maxCount` | Number | 最大上传张数。 |
| `caseUploadItems[].uploadedCount` | Number | 已上传张数。 |
| `vehicles` | Array | 车辆列表。 |
...
| `SCENE_45` | 后端按案件级现场照片配置映射 | 后端按案件级现场照片配置映射 | 1 | 案件级整车 45 度现场照片。 |
| `SCENE_SUPPLEMENT` | 后端按案件级现场补充照片配置映射 | 后端按案件级现场补充照片配置映射 | 3 | 案件级现场补充照片。 |
| `DAMAGE` | `02/0202/1201/CRPHTO` | `02/0203/1201/CRPHTO` | 10 | 车损照片。 |
| `DRIVER_LICENSE_FRONT` | 后端按驾驶证正页配置映射 | 后端按驾驶证正页配置映射 | 1 | 驾驶证正页。 |
| `DRIVER_LICENSE_BACK` | 后端按驾驶证副页配置映射 | 后端按驾驶证副页配置映射 | 1 | 驾驶证副页。 |
| `DRIVER_LICENSE_ELECTRONIC` | 后端按电子驾驶证配置映射 | 后端按电子驾驶证配置映射 | 1 | 电子驾驶证。 |
...
| `vehicleId` | String | 条件必填 | 车辆级照片必须传初始化返回的车辆唯一标识；`SCENE_45`、`SCENE_SUPPLEMENT` 等案件级照片的请求体不要包含该字段，不能传空字符串或 `null`。 |
...
| `sortNo` | Number | 否 | 同一上传项下的排序。车损照片使用 `1..10`，现场补充照片使用 `1..3`；现场主照片、车牌、VIN 和各类证件照片固定为 `1`。 |
...
```json
{
  ...
  "uploadItemId": "CASE_100001_SCENE_45",
  "photoType": "SCENE_45",
  "clientPhotoId": "mini-local-scene-45",
  "sortNo": 1,
  "fileName": "scene-45.jpg",
  "fileBase64": "/9j/4AAQSkZJRgABAQ..."
}
```
...
2. 后端会根据 `ticket + uploadItemId + clientPhotoId/fileHash` 做幂等控制；车辆级照片额外校验 `vehicleId`，案件级照片不得要求或接收 `vehicleId`。
3. 后端必须校验该图片是否属于 ticket 初始化返回的案件级或车辆级拍照项。
4. 上传响应不返回真实 `objectKey`、`fileServerAddr`。如需服务端图片预览，应通过后端代理预览接口或短期签名 URL 实现。
5. 后端必须校验上传数量不能超过初始化返回的 `maxCount`，例如案件级现场照片 1 张、现场补充照片最多 3 张、同一车辆车损最多 10 张、车牌/VIN/驾驶证/行驶证正副页各 1 张。

## 8. 完成接口

客户完成本次拍照后，小程序调用完成接口。

本轮不修改 complete 接口路径、请求参数或前端处理逻辑；后端当前怎么返回，前端就按现有逻辑处理。本轮不要求后端新增 `requiredPassed` 或 `missingItems`。
...
| `VIN` | VIN 照片。 | 1 |
| `SCENE_45` | 案件级整车 45 度现场照片。 | 1 |
| `SCENE_SUPPLEMENT` | 案件级现场补充照片。 | 3 |
| `DAMAGE` | 车损照片。 | 10 |
| `DRIVER_LICENSE_FRONT` | 驾驶证正页。 | 1 |
| `DRIVER_LICENSE_BACK` | 驾驶证副页。 | 1 |
| `DRIVER_LICENSE_ELECTRONIC` | 电子驾驶证。 | 1 |
...
3. 上传图片时必须使用初始化接口返回的 `uploadItemId` 和 `photoType`；车辆级照片必须同时使用初始化返回的 `vehicleId`，案件级 `SCENE_45`、`SCENE_SUPPLEMENT` 请求体不得包含 `vehicleId`。
...
10. 图片上传一张一个请求；现场主照片使用 `sortNo=1`，现场补充照片最多 3 张并使用 `sortNo=1..3`，车损照片最多 10 张并使用 `sortNo=1..10`。
11. 驾驶证和行驶证均按正页、副页、电子证件拆分为独立 `uploadItemId`，不要用一个上传项承载多张图或多种类型。
...
| T-007 | 驾驶证或行驶证未上传是否阻断完成 | 当前口径为只提示缺失项，不硬性阻断完成。 |
