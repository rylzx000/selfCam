...
| `vehicleId` | String | 条件必填 | 车辆级照片必须传初始化返回的车辆唯一标识；`SCENE_45`、`SCENE_SUPPLEMENT` 等案件级照片不传或传空字符串。 |
...
2. 后端会根据 `ticket + uploadItemId + clientPhotoId/fileHash` 做幂等控制；车辆级照片额外校验 `vehicleId`。
...
10. 图片上传一张一个请求；车损照片最多 10 张，上传时用 `sortNo=1..10` 标识顺序。
