...
### nova13 / OpenHarmony 横屏

- 前置：使用高分辨率横屏设备或 OpenHarmony/OHOS 横屏设备；普通 iOS/Android 横屏仍应保持原有 `rpx` 展示。
- 期望：相机区保持 4:3，车牌和车损取景框位置不漂移，拍照页文字、按钮、提示条、边框和预览页缩略图、底部按钮、行驶证面板相对正常机型同步放大。
- 期望：正常横屏机型不启用该缩放分支，原有拍摄手感和显示尺寸不变。

### 非 4:3 实时帧自动拍照

- 前置：AI 检测输出来自非 4:3 实时帧，例如宽帧 `800 x 450` 或 nova13 日志中的正方形帧 `480 x 480`。
- 期望：宽帧按相机预览 aspect-fill 裁剪映射到虚拟 `400 x 300` 坐标；正方形/窄帧按高度贴合并左右补边映射，再进入取景框和稳定性判定。
- 期望：车牌、车损原自动拍照阈值不变；视觉上已进入取景框的目标可以继续累积稳定帧并触发自动拍照。

### 业务排障日志

- 前置：业务测试时记录控制台输出的 `selfCam_${sessionId}`，在微信小程序管理后台实时日志按该过滤号查询。
- 期望：能看到 `camera_layout_snapshot`、`preview_layout_snapshot`、`ai_geometry_snapshot`、`auto_capture_gate_sample` 或 `auto_capture_ready`。
- 期望：日志包含窗口/相机尺寸、UI 缩放分支、帧宽高、映射模式、取景框、映射后目标框、gate 状态和失败原因；不包含原始帧字节。
...
- 车牌/车损模型创建 session 前的 `wx.getInferenceEnvInfo` 只作为日志信息，失败或不支持不应阻断加载
...
- `npm run test:automator` 已覆盖车损步骤在 `cameraInitialized=false` 时等待 `onCameraInitDone()` 后再次尝试启动 AI 检测，并验证重复 `initdone` 不会启动多个检测循环。
...
## v1.3.4 确认态与 AI 恢复测试补充

本版本没有修改 AI 检测策略，但补充确认态边界对 AI 恢复的影响验证：

- VIN 确认切换到车损步骤后，仍会在 `setData` 完成后调用 `resumeAIDetectionAfterStepReady('confirm_vin_to_damage')`。
- 相册保存返回失败结果时，确认流程仍推进缓存和步骤，AI 恢复不被阻断。
- 用户点击 `重新拍摄` 时，不保存待确认照片到系统相册，并恢复当前步骤 AI 检测。
...
- 本版本不修改车牌、车损 AI 检测、锁定、冷却、自动拍照策略。
- 拍照页仅同步初始化页和预览页的浅灰绿色背景与右上淡绿色装饰，不改变相机覆盖层和 AI 恢复逻辑。
- 行驶证上传发生在预览页，使用 `wx.chooseMedia` 拍照/相册选择，不接入拍照页车牌/VIN/车损状态机。
