...
- 前置：使用高分辨率横屏设备或开发者工具等效窗口，触发布局比例 `layoutScale >= 1.3`。
- 期望：相机区保持 4:3，车牌和车损取景框位置不漂移，文字、按钮、提示条和边框相对正常机型同步放大。
- 期望：正常横屏机型不启用该缩放分支，原有拍摄手感和显示尺寸不变。

### 宽屏实时帧自动拍照

- 前置：AI 检测输出来自非 4:3 实时帧，例如 `800 x 450`。
- 期望：检测框先按相机预览 aspect-fill 裁剪映射到虚拟 `400 x 300` 坐标，再进入取景框和稳定性判定。
- 期望：车牌、车损原自动拍照阈值不变；视觉上已进入取景框的目标可以继续累积稳定帧并触发自动拍照。

### 业务排障日志

- 前置：业务测试时记录控制台输出的 `selfCam_${sessionId}`，在微信小程序管理后台实时日志按该过滤号查询。
- 期望：能看到 `camera_layout_snapshot`、`ai_geometry_snapshot`、`auto_capture_gate_sample` 或 `auto_capture_ready`。
- 期望：日志包含窗口/相机尺寸、UI 缩放分支、帧宽高、映射模式、取景框、映射后目标框和 gate 状态；不包含原始帧字节。
