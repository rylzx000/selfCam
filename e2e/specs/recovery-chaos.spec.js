...
describe('P0 恢复与乱序操作 e2e', () => {
  ...
  test('[P0-11] 删除/重拍后退出重进，cache 和页面统计仍然一致', async () => {
    ...
    let page = await miniProgram.reLaunch('/packageD/pages/preview/preview?mode=moduleTwo')
    await wait(800)
    ...
    page = await miniProgram.reLaunch('/packageD/pages/preview/preview?mode=moduleTwo')
    await wait(900)
