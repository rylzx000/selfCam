...
**版本号**: v1.5.7
**发布日期**: 2026-07-24
**状态**: 待提交：证件补录入口与完成态规则修复

---

## 版本概述

当前版本修复驾驶证和行驶证删除或部分补录后缺失正页/副页入口可能消失的问题，并修正电子证件 / 实物证件二选一完成规则。
同时补齐 `E/F/B` 八状态、模块三与最终预览入口全组合、删除形成缺失态后的补录入口全组合测试，避免证件入口在混合过渡态再次遗漏。

### 本版本重点

- 证件完成态按实际照片判断：电子证件存在，或实物正页 + 副页同时存在，即视为完成。
- `documentSelections` 仅表示最近一次成功上传方式，打开弹层或切换电子/实物面板不再改变完成态。
- 只有电子证件时不展示实物正页/副页补拍入口，实物正副页齐全时不展示电子证件入口。
- 电子证件 + 实物单侧过渡态仍展示缺失实物侧 `+`，便于用户补齐已开始的实物链路。
...
- 补充证件采集矩阵、页面操作链、模块三/最终预览入口全组合和删除后补录入口全组合 Jest 用例。
...
| 版本 | 发布日期 | 状态 | 说明 |
| --- | --- | --- | --- |
| v1.5.7 | 2026-07-24 | 待提交：证件补录入口与完成态规则修复 | 修复证件部分补录入口消失和误切完成态问题，补齐 E/F/B 矩阵、路由入口全组合和删除后补录入口测试 |
