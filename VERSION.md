...
**版本号**: v1.5.4
**发布日期**: 2026-07-17
**状态**: 待提交：模块二车损完成入口优化与质量门禁补强

---

## 版本概述

当前版本优化模块二车损连续拍摄完成入口，在保留连续拍摄效率和现有完成流转的基础上，明确告诉用户可结束当前车辆车损采集。
同时补充轻量质量门禁和破坏性压测覆盖地图，用于提前发现流程矩阵、预览跳转、过期文案、素材大小、Comet 临时产物、乱序恢复和上传中断类风险。

### 本版本重点

- 车损拍摄页现有主动流程按钮统一显示 `完成本车拍摄`。
- 车损数量展示改为 `本车已拍 x 张`，不展示 `x/10` 或剩余可拍数量。
- 当前车辆少于 3 张车损照片时点击完成，展示 `车损照片较少` 软确认；确认后仍可完成，不做硬拦截。
- 当前车辆第 4 张起，车损拍摄提示改为 `已完成基础采集，可点击完成本车拍摄`。
- 保持单车、多车、拍满 10 张、模块二预览、上传接口和后端接口流程稳定。
- 同步 `camera-capture` 主规格、OpenSpec 归档、弹窗清单、流程路由矩阵和相关 PRD/UI/技术文档。
- `package.json`、`package-lock.json`、`VERSION.md` 和辅助拍照请求 `CLIENT_VERSION` 提升到 `1.5.4`。
- 新增 `__tests__/quality-guards.test.js`，覆盖流程矩阵一致性、无 `mode` 老预览跳转、过期文案、分包素材和 Comet/OpenSpec 临时产物。
- 新增 `docs/destructive-stress-test-coverage.md`，归纳破坏性、异常、乱序、恢复、边界、接口失败和系统 API 失败测试覆盖。
- 补强模块二低数量软确认防重复推进、最终预览全局乱序删除补拍、上传中断恢复等 Jest 用例，并修复跟随 `CLIENT_VERSION` 的过期版本号断言。
