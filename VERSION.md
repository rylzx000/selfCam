...
当前版本在 `v1.4.9` 辅助拍照异常日志上报基础上，重组查勘采集流程为现场环境及车辆信息、车损照片拍摄、证件信息采集三个阶段，并补齐最终总预览和提交前确认链路。

### 本版本重点

- 新增案件级现场环境照片采集，支持整车 45 度现场照片和现场补充照片。
- 模块一采集现场环境、受损车辆车牌号和 VIN，完成后进入模块一预览页。
- 模块二直接进入车损照片拍摄，车损照片扩展到每车最多 10 张。
- 模块三按车辆维度采集驾驶证和行驶证，支持电子证件和实物正页 / 副页。
- 最终总预览按三阶段分组展示所有采集内容，并保持提交前统一确认。
