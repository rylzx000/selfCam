...
**版本号**: v1.5.5
**发布日期**: 2026-07-21
**状态**: 待提交：预览恢复与补拍入口可见性修复

---

## 版本概述

当前版本修复现场类照片语境、最终预览车损补拍入口、证件多模式汇总展示，以及同 ticket 二次进入时模块化预览步骤被安全恢复吞掉的问题。
同时补充真实 storage 安全恢复路径测试和相机页非拍摄步骤防御，避免 mock 测试通过但真实恢复链路仍丢失预览 mode 的回归。

### 本版本重点

- 现场 45 度和现场补充照片拍摄页左侧信息改为案件级“现场照片”，不再误展示“当前车辆”语境。
- 最终预览页删除车牌号、VIN 和全部车损照片后，车损照片区域仍保留可点击补拍入口。
- 驾驶证和行驶证同时存在实物正页、实物副页和电子版时，模块三预览页和最终预览页按固定顺序展示全部已采集照片。
- storage 安全恢复在预览中状态下保留 `moduleOnePreview`、`moduleThree`、`finalPreview` 检查点，首页恢复到对应 `mode` 预览页。
- 相机页加载预览型或未知步骤时执行兜底重定向，`onConfirmPhoto` 仅允许合法拍摄步骤保存照片。
- 同步 `camera-capture` 和 `preview-flow` 主规格，归档 `fix-preview-recovery-and-entry-visibility` OpenSpec change。
- `package.json`、`package-lock.json`、`VERSION.md` 和辅助拍照请求 `CLIENT_VERSION` 提升到 `1.5.5`。
- 补充最终预览补拍入口、证件多 side 汇总展示、真实 storage 安全恢复路径和相机页恢复防御相关 Jest 用例。
