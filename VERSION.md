...
**版本号**: v1.5.1
**发布日期**: 2026-07-10
**状态**: main 已包含：接入 OpenSpec 和 Comet

---

## 版本概述

当前版本在 `v1.5.0` 查勘采集三阶段流程基础上，接入 OpenSpec 和 Comet 项目工作流，补充现有文档基线索引与协作规则。

### 本版本重点

- 新增 OpenSpec 项目配置与 Codex 侧 OpenSpec 技能，用于后续规格驱动变更管理。
- 新增 Comet 项目配置与阶段守卫规则，用于规范后续多阶段任务执行。
- 新增 `baseline-existing-selfcam` OpenSpec 基线 change，索引现有 `docs/` 与 `PRDS/` 文档，不迁移原文全文。
- `AGENTS.md` 补充 OpenSpec / Comet 使用规则，以及中文正文、英文 kebab-case 标识的命名与语言规则。
