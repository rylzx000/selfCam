...
**版本号**: v1.5.2
**发布日期**: 2026-07-11
**状态**: 待提交：临时关闭车牌和车损 AI 自动拍照

---

## 版本概述

当前版本在 `v1.5.1` OpenSpec / Comet 工作流基础上，临时关闭车牌号和车损拍摄页面的 AI 识别及自动拍照能力，相关步骤统一退化为正常手动拍照模式。

### 本版本重点

- 新增集中式 AI 自动拍照业务开关，当前默认关闭车牌号和车损 AI。
- AI 关闭时不下载模型、不创建推理 session、不启动帧监听、检测循环或自动拍照定时器。
- 车牌号和车损拍摄按钮显示“点击拍照”，不展示 AI 初始化、识别中或 AI 不可用等状态提示。
- 保留车牌号手动拍照、确认、重拍和后续流转。
- 保留车损手动拍照、多车采集、最多 10 张、补拍和完成弹层流程。
- 归档 `disable-ai-auto-capture` OpenSpec change，并同步 `ai-auto-capture` 主规格。
- `AGENTS.md` 补充提交前版本号与中文提交备注建议、项目文字内容中文优先等协作规则。
