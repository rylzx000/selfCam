...
- 修复驾驶证和行驶证删除或部分补录后，缺失正页/副页补拍入口可能消失的问题。
- 修正证件完成规则：电子证件 1 张，或实物正页 + 副页 2 张满足其一即可完成，不要求三张全部补齐。
- 调整证件弹层写入时机，打开弹层或切换电子/实物面板不再改变 `documentSelections`，仅上传成功后更新最近采集方式。
- 修复电子证件 + 实物单侧的过渡态：提交态保持完成，同时仍允许补齐缺失实物侧。

### 文档

- 归档 `fix-document-partial-recapture` OpenSpec / Comet hotfix change，并同步 `preview-flow` 主规格。
- 同步 PRD、UI、技术文档、测试运行指引、流程路由矩阵和弹窗交互清单中的证件采集矩阵规则。

### 测试

- 补充驾驶证 / 行驶证 `E/F/B` 八状态工具层矩阵测试。
- 补充模块三与最终预览证件入口全组合路由测试：`moduleThree/final × 驾驶证/行驶证 × F/B/E+F/E+B`。
- 补充删除形成缺失态后的补录入口全组合测试，覆盖删除后点击缺失侧 `+` 是否打开实物证件面板并保持当前预览上下文。
- 已通过相关 JS 语法检查、OpenSpec strict validation、指定 Jest 回归集和 Comet verify/archive。

### 版本

- `package.json`、`package-lock.json`、`VERSION.md` 和辅助拍照请求 `CLIENT_VERSION` 提升到 `1.5.7`。
...
- 修复整车 45 度和现场补充照片拍摄页左侧仍展示“当前车辆”的语境问题，现场类照片改为案件级“现场照片”提示。
- 修复最终预览页删除车牌号、VIN 和全部车损照片后，车损照片区域补拍入口可能消失的问题。
- 修复同一车辆同一证件同时存在电子版、实物正页和实物副页时，模块三预览页和最终预览页只展示最后拍摄模式的问题。
- 修复同 ticket 二次进入时，安全恢复将模块化预览步骤规整成旧预览步骤，导致首页丢失模块化预览路由的问题。
- 相机页增加非拍摄步骤恢复防御，避免未知或预览型步骤误把照片保存成车损。

### 文档

- 归档 `fix-preview-recovery-and-entry-visibility` OpenSpec change，并同步 `camera-capture`、`preview-flow` 主规格。
- 同步 PRD 和 UI 文档中现场照片语境、补拍入口、证件汇总展示和预览恢复规则。

### 测试

- 补充最终预览车损补拍入口可见性、证件多 side 汇总展示、删除单张证件保留其余证件、真实 storage 安全恢复路径、相机页非拍摄步骤防御和多车恢复不串车测试。
- 已通过相关 Jest、JS 语法检查、OpenSpec strict validation、Comet verify/archive、`git diff --check` 和轻量质量门禁。

### 版本

- `package.json`、`package-lock.json`、`VERSION.md` 和辅助拍照请求 `CLIENT_VERSION` 提升到 `1.5.5`。

## [v1.5.4] - 2026-07-17

### 调整

- 模块二车损拍摄页现有完成入口统一为 `完成本车拍摄`，明确表达当前车辆车损采集结束。
- 车损数量展示调整为 `本车已拍 x 张`，不再展示 `x/10` 或剩余可拍数量，弱化 10 张目标感。
- 当前车辆少于 3 张车损照片时点击完成，先展示 `车损照片较少` 软确认；确认后仍复用原完成本车流程，不硬拦截。
- 当前车辆第 4 张起，车损拍摄提示切换为 `已完成基础采集，可点击完成本车拍摄`。
- 保持单车、多车、拍满 10 张、模块二预览和上传接口流程稳定。
...
- `package.json`、`package-lock.json`、`VERSION.md` 和辅助拍照请求 `CLIENT_VERSION` 提升到 `1.5.4`。
...
- 临时关闭车牌号和车损拍摄页面的 AI 识别及自动拍照能力，相关步骤统一退化为手动拍照模式。
- 新增集中式 AI 自动拍照业务开关，当前默认关闭；后续恢复时可通过配置重新启用。
- AI 关闭时不再下载模型、创建推理 session、启动帧监听、检测循环或自动拍照定时器。
- 车牌号和车损拍摄按钮统一显示“点击拍照”，不展示 AI 初始化、识别中或 AI 不可用等状态提示。

### 保留

- 保留车牌号手动拍照、确认、重拍和后续流转。
- 保留车损手动拍照、多车采集、最多 10 张、补拍和完成弹层流程。

### 文档

- 归档 `disable-ai-auto-capture` OpenSpec change，并同步 `ai-auto-capture` 主规格。
- 补充项目协作规则：提交前需给出版本号与中文提交备注建议，项目文字内容优先使用中文。

### 测试

- 补充 AI 总开关关闭、检测器不加载、推理 session 不创建、AI 流程不启动、手动拍照流程正常等单测覆盖。
- 已通过相关 Jest 测试、JS 语法检查和 OpenSpec strict validation。

### 版本

- `package.json`、`package-lock.json`、`VERSION.md` 和辅助拍照请求 `CLIENT_VERSION` 提升到 `1.5.2`。
...
- 新增查勘采集三阶段流程：现场环境及车辆信息、车损照片拍摄、证件信息采集，并串联最终总预览和提交链路。
- 新增案件级现场环境照片采集，支持整车 45 度现场照片和现场补充照片。
- 新增驾驶证采集能力，驾驶证和行驶证均按车辆维度展示，支持电子证件和实物正页 / 副页。
- 新增多模式预览页 HTML 原型和模块一流程原型，用于对齐横屏小程序交互。

### 调整

- 模块一完成后进入本环节预览页，再通过阶段完成弹层进入车损拍摄。
- 模块二直接进入车损照片拍摄，不再重复拍摄车牌号和 VIN。
- 模块三改为证件信息页，通过车辆卡片内的加号入口补充证件，不再单独设置线性拍摄流程。
- 最终总预览按流程分组展示现场环境、车辆识别信息、车损照片和证件信息。
- 车损照片容量扩展到每车最多 10 张，并调整多车场景下的展示和跳转。
- 更新 pilot / prod 环境端点配置，并忽略本地打包产物 `packageD.zip`。

### 测试

- 补充模块一预览、模块一到模块二流转、模块三证件展示、缓存结构、上传状态和多车流程相关测试。
- 扩展微信开发者工具自动化夹具，支持三阶段流程验证。

### 版本

- `package.json`、`package-lock.json`、`VERSION.md` 和辅助拍照请求 `CLIENT_VERSION` 提升到 `1.5.0`。
...
- 接入辅助拍照 `uploadPhotoBase64` 逐张上传和 `complete` 完成接口，上传全部成功后才提交完成，完成成功后进入完成页。
- 新增本地辅助拍照 mock 后端脚本，可验证上传失败和完成接口重试。
...
| 版本 | 日期 | 类型 | 说明 |
| --- | --- | --- | --- |
| v1.4.6 | 2026-05-27 | 辅助拍照后端提交闭环版 | 接入真实 `uploadPhotoBase64` 逐张上传、`complete` 完成提交、本地 mock 后端和失败重试恢复 |
